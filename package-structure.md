# Overview
This document defines a reusable structure for Python packages that are installed from GitHub via `pip` and used inside a temporary virtual environment for the current shell session.
...
- `tool.setuptools.packages` (or equivalent backend config) must include the import package directory.

2. `<package_name>/__init__.py`
...
2. Ensure package directory name matches import path.
3. Export intended API from `<package_name>/__init__.py`.
